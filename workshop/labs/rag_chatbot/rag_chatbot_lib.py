...
def convert_chat_messages_to_converse_api(chat_messages):
    ...
    for chat_msg in chat_messages:
        messages.append({"role": chat_msg.role, "content": [{"text": chat_msg.text}]})
...
def process_tool(response_message, messages, bedrock, tool_list):
        ...
        if "toolUse" in content_block:
            ...
            if tool_use_block["name"] == "get_amazon_bedrock_information":
                collection = get_collection(
                    str(SCRIPT_DIR / "../../data/chroma"), "bedrock_faqs_collection"
                )
    ...
    if len(follow_up_content_blocks) > 0:
        ...
        return True, response["output"]["message"]["content"][0][
            "text"
        ]  # tool used, response
